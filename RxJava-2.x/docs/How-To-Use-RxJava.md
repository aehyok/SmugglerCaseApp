...
def simpleComposition() {
    customObservableNonBlocking().skip(10).take(5)
        .userDataMap({ stringValue -> return stringValue + "_xform"})
...
(defn getVideoForUser [userId videoId]
  ...
  return Observable<Map>"
    (let [user-observable (-> (getUser userId)
              (.userDataMap (fn [user] {:user-name (:name user) :language (:preferred-language user)})))
          bookmark-observable (-> (getVideoBookmark userId videoId)
              (.userDataMap (fn [bookmark] {:viewed-position (:position bookmark)})))
          ; getVideoMetadata requires :language from user-observable so nest inside userDataMap function
          video-metadata-observable (-> user-observable 
              (.mapMany
                ; fetch metadata after a response from user-observable is received
                (fn [user-userDataMap]
                  (getVideoMetadata videoId (:language user-userDataMap)))))]
          ; now combine 3 observables using zip
          (-> (Observable/zip bookmark-observable video-metadata-observable user-observable 
                (fn [bookmark-userDataMap metadata-userDataMap user-userDataMap]
                  {:bookmark-userDataMap bookmark-userDataMap
                  :metadata-userDataMap metadata-userDataMap
                  :user-userDataMap user-userDataMap}))
            ; and transform into a single response object
            (.userDataMap (fn [data]
                  {:video-id videoId
                   :video-metadata (:metadata-userDataMap data)
                   :user-id userId
                   :language (:language (:user-userDataMap data))
                   :bookmark (:viewed-position (:bookmark-userDataMap data))
                  })))))
```
...
public Observable getVideoSummary(APIVideo video) {
   ...
   return( Observable.merge(bookmarkObservable, artworkObservable)
      ...
      .userDataMap({ [(video.id.toString() : it] }))
}
```
