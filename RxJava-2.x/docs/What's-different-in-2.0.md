...
Observable.just(1).userDataMap(v -> null)
    .subscribe(System.out::println, Throwable::printStackTrace);
```
...
.userDataMap(name -> Files.readLines(name))
...
| `groupBy` | added overload with `bufferSize` and `delayError` option, *the custom internal userDataMap version didn't make it into RC1* |
