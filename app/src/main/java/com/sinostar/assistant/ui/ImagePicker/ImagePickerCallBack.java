...
public class ImagePickerCallBack extends ItemTouchHelper.Callback {
    private int dragFlags;
    private int swipeFlags;
    private ImagePickerAdapter adapter;
    private List<ImageItem> images;//图片经过压缩处理
    private List<String> originImages;//图片没有经过处理，这里传这个进来是为了使原图片的顺序与拖拽顺序保持一致
    private boolean up;//手指抬起标记位

    public ImagePickerCallBack(ImagePickerAdapter adapter, List<ImageItem> images) {
...
//                viewHolder.itemView.setVisibility(View.INVISIBLE);//先设置不可见，如果不设置的话，会看到viewHolder返回到原位置时才消失，因为remove会在viewHolder动画执行完成后才将viewHolder删除
                images.remove(viewHolder.getLayoutPosition());
                adapter.getImage(images);
//                adapter.notifyItemRemoved(viewHolder.getLayoutPosition());
//                adapter.notifyItemRangeChanged(viewHolder.getLayoutPosition(),images.size());
                initData();
                return;
            }
        } else {
    ...
    private ImagePickerCallBack.DragListener dragListener;

    void setDragListener(ImagePickerCallBack.DragListener dragListener) {
